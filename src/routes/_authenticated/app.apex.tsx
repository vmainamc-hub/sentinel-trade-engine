...
import PriceActionPressurePanel from "@/components/apex/PriceActionPressurePanel";
import DecisionSpinePanel from "@/components/apex/DecisionSpinePanel";
...
import { ObservationUniverseOverview } from "@/components/apex/ObservationUniverseOverview";
import {
  AlertHistoryPanel,
  AlertSettingsPanel,
...
function ApexPage() {
  ...
  const alerts = useOpportunityAlerts(apex.ranked, apex.surfacedOpportunity);
  ...
  const alertedLive = useMemo(() => {
    ...
    if (!live) return null;
    if (!apex.surfacedOpportunity) return null;
    if (opportunityKey(apex.surfacedOpportunity) !== opportunityKey(live)) return null;
  ...
  // the underlying authoritative finalRank remains unchanged and auditable.
  ...
  const best = selectScanCandidate(apex.scan, apex.ranked, apex.surfacedOpportunity);
  ...
  const handleScan = useCallback(() => {
    ...
    // Scan must interrogate the same surfaced candidate that the live UI uses.
    const scanCandidate = result.surfacedOpportunity ?? apex.surfacedOpportunity;
    if (scanCandidate) {
      void askAI(scanCandidate, result.globalDanger, (result.finalRank ?? []).slice(1, 3));
    } else {
      setReasoning(null);
    }
  }, [apex, askAI]);
      ...
      <div className="mx-auto max-w-[1500px] space-y-5 p-4 md:p-6">
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList>
            ...
            <TabsTrigger value="observation">90-Cell Universe</TabsTrigger>
                ...
                <Accordion type="multiple" className="space-y-3">
                  ...
                  >
                    ...
                    <AccordionContent className="space-y-4 pb-4">
                      <PanelBoundary label="Verdict & staged reasoning">
                  ...
                  >
                    ...
                    <AccordionContent className="space-y-5 pb-4">
                      <PanelBoundary label="Digit psychology & pressure">
                        <DigitPsychologyPanel item={best} />
                        <DecisionSpinePanel item={best} />
                        <PriceActionPressurePanel item={best} />
                      ...
                      <QualityMetrics
                        ...
                        onAsk={() =>
                          askAI(
                            best,
                            apex.globalDanger,
                            (apex.scan?.finalRank ?? apex.scan?.top ?? []).slice(1, 3),
                          )
                        }
                      />
                    </AccordionContent>
                  ...
                  >
                    ...
                    <AccordionContent className="space-y-5 pb-4">
                      <PanelBoundary label="Forward projection & exposure">
                  ...
                  >
                    ...
                    <AccordionContent className="pb-4">
                      <PanelBoundary label="Simulator detail">
                      <SimulatorWindows
                        symbol={best.symbol}
                        name={best.name}
                  ...
                  >
                    ...
                    <AccordionContent className="space-y-5 pb-4">
                      <WhyNotRunnerUp
                        top={best}
                        runners={(apex.scan?.finalRank ?? apex.scan?.top ?? []).slice(1, 3)}
                      />
                      <div className="grid gap-4 lg:grid-cols-2">
                        {(apex.scan?.finalRank ?? apex.scan?.top ?? []).slice(1, 3).map((r, i) => (
                          <RunnerUp
          ...
          <TabsContent value="simulator" className="mt-5 space-y-5">
            <PanelBoundary label="Simulator command centre">
              <SimulatorCommandCenter />
            </PanelBoundary>
            <PanelBoundary label="Simulator">
              <SimulatorPanel />
            </PanelBoundary>
          </TabsContent>

          <TabsContent value="entry" className="mt-5">
            <PanelBoundary label="Entry lab">
              <EntryConditionLab item={best ?? null} />
            </PanelBoundary>
          </TabsContent>

          <TabsContent value="learning" className="mt-5 space-y-5">
            <PanelBoundary label="Governance">
              <GovernancePanel best={best ?? null} />
            </PanelBoundary>
            <PanelBoundary label="What Sentinel has learned">
              <WhatSentinelLearned />
            </PanelBoundary>
            <PanelBoundary label="Learning dashboard">
              <LearningDashboard />
            </PanelBoundary>
          </TabsContent>

          <TabsContent value="observation" className="mt-5 space-y-5">
            <PanelBoundary label="90-Cell universe">
              <ObservationUniverseOverview />
            </PanelBoundary>
          </TabsContent>

          <TabsContent value="execution" className="mt-5">
            <PanelBoundary label="Execution & journal">
        ...
        <Stat
          ...
          value={
            apex.observationHealth
              ? `${apex.observationHealth.status} (${apex.observationHealth.cellsRipe ?? 0} RIPE)`
              : "ONLINE (90 CELLS)"
          }
          color={
  ...
  return (
    <div className="glass rounded-xl border border-border/50 p-10 text-center">
      ...
      <h2 className="font-display text-base font-semibold">
        {apex.online === 0 ? "DATA UNAVAILABLE" : "AWAITING OBSERVATION QUALIFICATION"}
      </h2>
      <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
        {apex.online === 0
          ...
          : "Markets are streaming and continuously observed across all 90 cells. No candidate has cleared the full observation exam yet (RIPE state, hard-veto-free, MODERATE+ quality). The observation layer only surfaces verified, high-conviction setups to eliminate false signals and rapid flickering."}
      </p>
    </div>
  );
}
