...
export default function StagedVerdict({ item }: { item: RankedOpportunity }) {
  ...
  const setup = (item as any)?.setup;
  const danger = (item as any)?.dangerComposition;
  const gate = (item as any)?.entryClearance;
  const combination = (item as any)?.combination ?? {};
  const combo = combination.exact;

  // Partial / legacy candidate shapes must not blank the panel.
  if (!direction || !setup || !danger || !gate || !combo) {
    return (
      <section className="mt-6 rounded-lg border border-border/60 bg-background/40 p-4">
        ...
        <p className="mt-2 text-[11px] text-muted-foreground">
          Staged reasoning is not available for this candidate yet — the scan has not produced a
          complete direction / setup / clearance record.
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="mt-6 rounded-lg border border-border/60 bg-background/40 p-4">
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-2">
        <div>
          ...
          <ul className="mt-2 space-y-1">
            {(direction.votes ?? []).slice(0, 8).map((v: any) => (
      ...
      {(danger.components?.length ?? 0) ? (
        <div className="mt-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
            Stage 2a danger composition
          </p>
          <ul className="mt-2 space-y-1">
            {(danger.components ?? []).map((d: any) => (
