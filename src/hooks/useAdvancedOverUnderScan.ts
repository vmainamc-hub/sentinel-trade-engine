      ...
      raf = requestAnimationFrame(() => {
        ...
        for (const s of SCAN_SYMBOLS) {
          ...
          // (7,8,9) suppressed but quietly rising → expect shift > 2.
          {
            ...
            const highsCount = [pct[7] ?? 0.1, pct[8] ?? 0.1, pct[9] ?? 0.1].filter(
              (p) => p < 0.105,
            ).length;
            ...
            if (allOk) {
              const base = 65;
              const lowEdge = Math.min(
                15,
                ((pct[0] ?? 0.1) + (pct[1] ?? 0.1) + (pct[2] ?? 0.1) - 0.33) * 100,
              );
              ...
              const sig: AdvancedSignal = {
                ...
                redDigits: [5, 7, 9],
                buildup: [
                  { digit: 7, pct: pct[7] ?? 0.1, slope: sl7 },
                  { digit: 8, pct: pct[8] ?? 0.1, slope: sl8 },
                  { digit: 9, pct: pct[9] ?? 0.1, slope: sl9 },
                ],
                exhaustion: [
                  { digit: 0, pct: pct[0] ?? 0.1, flat: flat0 },
                  { digit: 1, pct: pct[1] ?? 0.1, flat: flat1 },
                  { digit: 2, pct: pct[2] ?? 0.1, flat: flat2 },
                ],
          ...
          // (0,1,2) suppressed but quietly rising → expect shift < 7.
          {
            ...
            const lowsCount = [pct[0] ?? 0.1, pct[1] ?? 0.1, pct[2] ?? 0.1].filter(
              (p) => p < 0.105,
            ).length;
            ...
            if (allOk) {
              const base = 65;
              const highEdge = Math.min(
                15,
                ((pct[7] ?? 0.1) + (pct[8] ?? 0.1) + (pct[9] ?? 0.1) - 0.33) * 100,
              );
              ...
              const sig: AdvancedSignal = {
                ...
                redDigits: [0, 2, 4],
                buildup: [
                  { digit: 0, pct: pct[0] ?? 0.1, slope: sl0 },
                  { digit: 1, pct: pct[1] ?? 0.1, slope: sl1 },
                  { digit: 2, pct: pct[2] ?? 0.1, slope: sl2 },
                ],
                exhaustion: [
                  { digit: 7, pct: pct[7] ?? 0.1, flat: flat7 },
                  { digit: 8, pct: pct[8] ?? 0.1, flat: flat8 },
                  { digit: 9, pct: pct[9] ?? 0.1, flat: flat9 },
                ],
                exhaustionStatus: exhaustConfirmed ? "CONFIRMED" : "FORMING",
                momentum: "BEARISH",
                entryPrice,
