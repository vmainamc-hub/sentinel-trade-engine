...
/** True once the browser has allowed audio playback for this tab. */
export function isAlertAudioUnlocked() {
  return unlocked && sharedCtx?.state === "running";
}

/**
 * Called from a real user gesture (click) to satisfy autoplay policy.
 * Resolves true when audio is playable; optionally plays a confirmation chime.
 */
export async function primeAlertAudio(withTestSound = true): Promise<boolean> {
  const ctx = ensureCtx();
  if (!ctx) return false;
  try {
  ...
  unlock();
  if (withTestSound) playBeep();
  return ctx.state === "running";
}

/** Fire the alert chime immediately (used for manual tests). */
