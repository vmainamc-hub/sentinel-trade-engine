  ...
  try {
    new Notification("🚨 HIGH-QUALITY OPPORTUNITY DETECTED", {
      body:
        ...
        `Quality: ${s.qualityBand} · Entry: ${s.entryDigit ?? "—"} · Opportunity: ${s.score} · Entry confidence: ${s.confidence}\n` +
...
export function useOpportunityAlerts(
  ...
  surfacedOpportunity: RankedOpportunity | null = null,
): OpportunityAlertsState {
  ...
  useEffect(() => {
    ...
    const prevEp = stateRef.current.episode;
    const nextEp = next.episode;
    const episodeChanged =
      (prevEp === null) !== (nextEp === null) ||
      (prevEp !== null && nextEp !== null && (
        prevEp.key !== nextEp.key ||
        prevEp.status !== nextEp.status ||
        prevEp.entryDigit !== nextEp.entryDigit ||
        prevEp.state !== nextEp.state ||
        prevEp.alerts !== nextEp.alerts ||
        prevEp.peakScore !== nextEp.peakScore ||
        prevEp.closedAt !== nextEp.closedAt
      ));

    if (episodeChanged || fired.length > 0 || stateRef.current.history !== next.history) {
      setState(next);
    }
  ...
  // Legacy alert episodes remain available for the Alerts tab. The operator
  // sound is owned by the canonical surface-vetted signal in useApexSentinel.
  void surfacedOpportunity;
