...
function scoreCompressingBelow7(ctx: HypothesisContext): {
  ...
  const zoneUnder = [0, 1, 2, 3, 4, 5, 6].reduce((a, d) => a + (stats.pct?.[d] ?? 0.1), 0);
  const zoneOver = 1 - zoneUnder;
  // Digit 9 exhausted and decreasing (ceiling exhaustion).
  const d9 = stats.profiles?.[9] ?? { pct: stats.pct?.[9] ?? 0.1, pressure: 0 };
  const d8 = stats.profiles?.[8] ?? { pct: stats.pct?.[8] ?? 0.1, pressure: 0 };
...
function scoreBouncingFromFloor(ctx: HypothesisContext): {
  ...
  const floor = [0, 1, 2];
  const floorPct = floor.reduce((a, d) => a + (stats.pct?.[d] ?? 0.1), 0);
  const floorReleasing = floor.filter((d) => (stats.profiles?.[d]?.pressure ?? 0) <= 0.005).length;
  const upperBuilding = [7, 8, 9].filter((d) => (stats.profiles?.[d]?.pressure ?? 0) >= 0).length;
...
function scoreRotatingUpward(ctx: HypothesisContext): {
  ...
  const upperPressure = [5, 6, 7, 8, 9].reduce(
    (a, d) => a + (stats.profiles?.[d]?.pressure ?? 0),
    0,
  );
  const lowerPressure = [0, 1, 2, 3, 4].reduce(
    (a, d) => a + (stats.profiles?.[d]?.pressure ?? 0),
    0,
  );
