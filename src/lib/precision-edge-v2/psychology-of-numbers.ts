...
//                                     Digit 9 is a losing digit; only valid if ≥10.3%
...
//   • Light-Red (2nd least)         → MUST be in the winning zone.
//   • Purple (fastest growing)      → MUST be in the winning zone.
//   • Digit 0                       → MUST be ≥ 10.3% AND pressure ≤ 0 (decreasing).
...
export function psychologyReview(v: ContractVerdict, stats: DigitStatistics): PsychologyReview {
  ...
  // ── LAW 5E: UNDER — Digit 9 must be ≥10.3% AND decreasing ───────────
  // Operator rule: digit 9 must already be elevated (≥10.3%) AND its
  ...
  if (v.side === "UNDER") {
    ...
    const belowSoft = d9.pct < cfg.anchorExhaustionPct; // <10.3%
  ...
  // ── LAW 5F: OVER — Digit 0 must be ≥10.3% AND decreasing ────────────
  if (v.side === "OVER") {
